...
#define MFRC522_MAX_LEN 64
#define MAX_CARDS 20 // Maximum number of cards to store

/* RFID Card Data Structure */
typedef struct
{
    uint8_t uid[4];     // Card UID (4 bytes)
    uint8_t tagType[2]; // Card type
    uint8_t status;     // Read status: 1 = success, 0 = no card/error
} RFID_CardData;

/* RFID Card List Structure */
typedef struct
{
    RFID_CardData cards[MAX_CARDS]; // Array of stored cards
    uint8_t count;                  // Number of cards stored
} RFID_CardList;

/* RFID Handler Structure with function pointers (Application Layer) */
typedef struct
{
    // Data members
    RFID_CardList cardList;

    // Function pointers for application-level operations
    void (*init)(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin);
    RFID_CardData (*read_card)(void);
    uint8_t (*check_uid_exists)(uint8_t *uid);
    uint8_t (*add_card)(RFID_CardData *card);
    uint8_t (*remove_card)(uint8_t *uid);
    void (*clear_list)(void);
    uint8_t (*get_card_count)(void);
    RFID_CardData *(*get_card_by_index)(uint8_t index);
} RFID_Handler;

extern RFID_Handler RFID;
